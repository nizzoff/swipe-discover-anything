...
export function ItemTile({ item, onRemove }: { item: DiscoverItem; onRemove?: () => void }) {
  return <article className="group relative overflow-hidden rounded-3xl bg-card ring-1 ring-border transition duration-300 hover:-translate-y-1 hover:shadow-xl">
    <img src={item.image} alt={item.title} loading="lazy" width={1024} height={1024} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-105" />
