...
export function SettingsPanel() {
  return <section className="overflow-hidden rounded-[1.5rem] bg-card/70 ring-1 ring-border backdrop-blur-xl">
    {rows.map(({ icon: Icon, label, control, detail }, index) => <div key={label} className={`grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 p-4 ${index ? "border-t border-border" : ""}`}>
      <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary/10 text-primary ring-1 ring-primary/15"><Icon className="h-4.5 w-4.5" /></span>
