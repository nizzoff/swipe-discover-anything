...
function FavoritesPage() {
  ...
  const shown = useMemo(() => [...favorites.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()))].sort((a, b) => sort === "rating" ? b.rating - a.rating : sort === "alpha" ? a.title.localeCompare(b.title) : 0), [favorites, query, sort]);
  return <PaywallGuard><SwipeItShell wide><PageHeader eyebrow="Votre collection" title="Favoris" action={<span className="rounded-full bg-primary/15 px-3 py-1.5 text-xs font-bold text-primary">{favorites.length}</span>} />
